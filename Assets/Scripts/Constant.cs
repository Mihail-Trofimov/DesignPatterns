namespace Asteroids
{
    public static class Constant
    {
        public const string HORIZONTAL = "Horizontal";
        public const string VERTICAL = "Vertical";
        public const string FIRE1 = "Fire1";

        public const string TAG_PLAYER = "Player";

        public const string PREFAB_PLAYER = "Prefabs/Player";
        public const string PREFAB_GAME_LOOP = "Prefabs/GameLoop";
        public const string PREFAB_AMMUNITION_PLAYER_BLASTER = "Prefabs/PlayerBlaster";
        public const string PREFAB_SPAWN_ASTEROID = "Prefabs/SpawnerAsteroid";
        public const string PREFAB_SPECIAL_EFFECT_BLASTER = "Prefabs/SpecialEffectBlaster";

        public const string SPRITE_PLASMA = "Sprites/plasma";

        public const string NAME_POOL_PLAYER_BLASTER = "[PoolPlayerBlaster]";
        public const string NAME_POOL_BLASTER_EFFECTS = "[PoolSpecialEffectsBlaster]";
        public const string NAME_ASTEROIDS = "[Asteroids]";
        public const string NAME_ASTEROID = "Asteroid";
        public const string NAME_BLASTER = "Blaster";
    }
}